...
describe("Tester at komponenten rendres slik den skal", () => {
  ...
  test("Komponenten rendres dersom brukeren KAN reaktiveres og er nivå 4", async () => {
    ...
    expect(
      screen.getByText(/dersom du ønsker arbeidsrettet oppfølging fra NAV, må du være registrert som arbeidssøker\./i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/dersom du har søkt eller ønsker å søke om dagpenger må du være registrert som arbeidssøker\./i)
    ).toBeInTheDocument();
